...
import { LogdataForm } from './logdata-form.component';
import { Logdata } from './logdata';
import { LogdataInfo } from './logdata-info';
...
/**
 * @description: Coordinator quản lý bảng 'Logdata'
 */
export class LogdataComponent implements OnInit {
  ...
  filterObject: Logdata = new Logdata();
  //.
  // >checker.
  ...
  /**
   * @description: Hàm gọi dữ liệu bảng theo số trang
  ...
  search(filterObject: any, page: number) {
    let filterObjectT = JSON.parse(localStorage.getItem(Contants.LOG_DATA))
    if (Contants.isEmptyObject(filterObject) || Contants.isEmptyObject(filterObjectT)) {
      if(Contants.isEmptyObject(filterObject)) {
        localStorage.setItem(Contants.LOG_DATA, JSON.stringify(filterObject))
      } else {
  ...
  /**
   * @description: Hàm khởi tạo phiên bản dịch vụ
   */
  ...
  /**
   * @description: Hàm khởi tạo mã lỗi
   */
